...
Matrix4x4 OrbitingCamera::getScale()
{
    REAL height = settings.cameraFov * M_PI / 180.0;
    REAL width = atan(m_aspectRatio * tan(height/2.0)) * 2.0;
    return getScaleMat(Vector4(1.0/(tan(width/2.0)*settings.cameraFar),
                               1.0/(tan(height/2.0)*settings.cameraFar),
                               1.0/settings.cameraFar,
                               1.0));
}
